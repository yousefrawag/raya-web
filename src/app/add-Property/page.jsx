...
const AddProperty = () => {
  ...
  const handleChange = (e) => {
    ...
    if (section === 'project' && field === 'imagesURLs') {
      setFormData((prev) => ({
        ...prev,
        project: { ...prev.project, imagesURLs: Array.from(files) },
      }));
    } else {
      setFormData((prev) => ({
        ...prev,
  ...
  const uploadFiles = async (files) => {
    ...
    for (const file of files) {
      ...
      uploaded.push({ fileURL: res.data.url, fileID: res.data.public_id });
    }
    return uploaded;
  };
  ...
  const handleSubmit = async () => {
    if (!formData.client.fullName || !formData.client.email || !formData.client.phone) {
      setError('يرجى ملء جميع الحقول المطلوبة في بياناتك الأساسية');
      setCurrentStep(1);
      return;
    }
    ...
    try {
      ...
      if (payload.project.imagesURLs && payload.project.imagesURLs.length > 0) {
      ...
      const response = await authFetch.post('/sharedProperty', payload);
      console.log('Response:', response.data);
      setIsSubmitted(true);
    } catch (err) {
      console.error('Submission error:', err);
      setError(err.response?.data?.error || 'حدث خطأ أثناء الإرسال، يرجى المحاولة لاحقاً');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* <div className="md:col-span-2">
            <label className="block text-sm font-semibold text-gray-700 mb-2">رفع صور المشروع (اختياري)</label>
            <input
              type="file"
              ...
              className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-amber-500 focus:ring-2 focus:ring-amber-200 transition outline-none bg-gray-50 hover:bg-white text-slate-800"
            />
            <p className="text-xs text-gray-500 mt-1">يمكنك رفع أكثر من صورة (حجم كل صورة لا يتجاوز 5 ميجابايت)</p>
          </div> */}
        </div>
      </div>
    );
  ...
  // ======== SEO & Schema ========
  const pageTitle = 'أضف عقارك مع منصة الراية | تسويق عقاري واستشارات هندسية';
  ...
  const keywords =
    'إضافة عقار, تسويق عقاري, بيع عقار, إيجار عقار, استشارات هندسية, منصة الراية, عقارات فلسطين, شقق للبيع, فلل للبيع, أراضي للبيع, مشاريع سكنية, استثمار عقاري, ضمان عقاري, تبديل عقارات, تسويق إلكتروني, عقارات, سوق العقارات, شراء عقار, تأجير عقار, استشارة فنية, هندسة معمارية, تصميم داخلي, دراسة جدوى, تقييم عقاري, تطوير مشاريع, إدارة مشاريع, عقارات تجارية, عقارات سكنية, مجمعات سكنية, أبراج سكنية, فيلات فاخرة, شقق فاخرة, أراضي استثمارية, عقارات للبيع في فلسطين, عقارات في رام الله, عقارات في القدس, عقارات في نابلس, عقارات في الخليل, عقارات في غزة, عقارات في بيت لحم, عقارات في أريحا, عقارات في طولكرم, عقارات في قلقيلية, عقارات في جنين, عقارات في سلفيت, عقارات في طوباس, عقارات في رام الله والبيرة, عقارات في القدس الشرقية, عقارات في الضفة الغربية, قطاع العقارات, سوق العقارات الفلسطيني, عقارك في فلسطين, منصة عقارية فلسطينية, خدمات عقارية, تسويق عقاري إلكتروني, إعلانات عقارية, عرض عقار, طلب عقار, استشارة عقارية, استشارة هندسية, مهندس معماري, مهندس إنشائي, تصميم معماري, إشراف هندسي, إدارة مشاريع عقارية, تطوير عقاري, استثمار عقاري في فلسطين, فرص استثمارية, عقارات للاستثمار, مشاريع سكنية جديدة, مشاريع تجارية, مشاريع سياحية, عقارات فاخرة, عقارات اقتصادية, عقارات متوسطة, شقق سكنية, فلل سكنية, أراضي زراعية, أراضي سكنية, عقارات صناعية, عقارات تجارية, محلات تجارية, مكاتب إدارية, مستودعات, شقق مفروشة, فلل مفروشة, عقارات للبيع بالتقسيط, عقارات للبيع نقداً, عقارات للإيجار طويل الأمد, عقارات للإيجار قصير الأمد, عقارات للإيجار السياحي, عقارات للتبديل, عقارات للضمان, عقارات للاستثمار طويل الأمد, استشارات فنية, استشارات هندسية, استشارات قانونية عقارية, استشارات مالية عقارية, تمويل عقاري, قروض عقارية, بنوك عقارية, شركات تمويل, شركات مقاولات, شركات تطوير عقاري, شركات وساطة عقارية, سماسرة عقارات, مكاتب عقارات, وكلاء عقاريون, إدارة أملاك, إدارة مجمعات سكنية, صيانة عقارات, تجديد عقارات, ترميم عقارات, بناء عقارات, تشطيب عقارات, ديكورات داخلية, ديكورات خارجية, تصميم حدائق, مساحات خارجية, تراسات, أسطح خضراء, مواقف سيارات, مصاعد, أمن وسلامة, أنظمة إطفاء, أنظمة إنذار, كاميرات مراقبة, بنية تحتية, مرافق عامة, خدمات عقارية متكاملة, منصة عقارية, الراية العقارية, الراية للتسويق العقاري, الراية للاستشارات الهندسية';
  const schemaData = {
    ...
    serviceType: ['تسويق عقاري', 'استشارات هندسية', 'بيع عقار', 'إيجار عقار', 'استثمار عقاري'],
    offers: {
      '@type': 'Offer',
    ...
    about: {
      ...
      description: 'خدمة عرض وإضافة العقارات والمشاريع السكنية والتجارية والصناعية في فلسطين مع استشارات هندسية متكاملة.',
    },
    audience: {
      '@type': 'Audience',
      audienceType: 'ملاك العقارات والمستثمرين والمطورين العقاريين',
    },
    keywords: keywords,
  };
  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:url" content="https://alraya.com/add-property" />
        <meta property="og:image" content="https://alraya.com/og-image.jpg" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={pageTitle} />
        <meta name="twitter:description" content={pageDescription} />
        <link rel="canonical" href="https://alraya.com/add-property" />
        <script
